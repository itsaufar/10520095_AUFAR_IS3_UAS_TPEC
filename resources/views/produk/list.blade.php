            ...
            <div class="col-lg-12">
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td>     
                            <a href="{{ route('produk.edit' , $item->id) }}" method="POST">
                            @csrf 
                            @method('DELETE')
                            <input type="submit" value="Hapus"/>
                        <a class= "bnt btn-warning btn-sm" href="{{ route('produk.edit', $item->id) }}">Edit</a>
                                
                            <form action="{{ route('produk.destroy', $item->id)}}" method="POST" class="id-inline formDelete">
                                @csrf 
                                @method('DELETE')
                                <a class="btn btn-danger btn-sm" type="submit">Hapus</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
